...
/// struct for typed errors of method [`engine_forks_voting_results_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum EngineForksVotingResultsPostError {
    Status500(models::UnexpectedError),
    UnknownValue(serde_json::Value),
}
...
pub async fn engine_configuration_post(configuration: &configuration::Configuration, engine_configuration_request: models::EngineConfigurationRequest) -> Result<models::EngineConfigurationResponse, Error<EngineConfigurationPostError>> {
...
pub async fn engine_forks_voting_results_post(configuration: &configuration::Configuration, forks_voting_results_request: models::ForksVotingResultsRequest) -> Result<models::ForksVotingResultsResponse, Error<EngineForksVotingResultsPostError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/engine/forks-voting-results", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&forks_voting_results_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<EngineForksVotingResultsPostError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}

pub async fn engine_status_post(configuration: &configuration::Configuration, engine_status_request: models::EngineStatusRequest) -> Result<models::EngineStatusResponse, Error<EngineStatusPostError>> {
    let local_var_configuration = configuration;
