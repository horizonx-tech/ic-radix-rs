...
/// struct for typed errors of method [`key_vote_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum KeyVotePostError {
    Status500(models::UnexpectedError),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`key_withdraw_vote_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum KeyWithdrawVotePostError {
    Status500(models::UnexpectedError),
    UnknownValue(serde_json::Value),
}


pub async fn key_list_post(configuration: &configuration::Configuration, key_list_request: models::KeyListRequest) -> Result<models::KeyListResponse, Error<KeyListPostError>> {
...
pub async fn key_sign_post(configuration: &configuration::Configuration, key_sign_request: models::KeySignRequest) -> Result<models::KeySignResponse, Error<KeySignPostError>> {
    let local_var_configuration = configuration;
...
pub async fn key_vote_post(configuration: &configuration::Configuration, update_vote_request: models::UpdateVoteRequest) -> Result<models::UpdateVoteResponse, Error<KeyVotePostError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/key/vote", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&update_vote_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<KeyVotePostError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}

pub async fn key_withdraw_vote_post(configuration: &configuration::Configuration, update_vote_request: models::UpdateVoteRequest) -> Result<models::UpdateVoteResponse, Error<KeyWithdrawVotePostError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/key/withdraw-vote", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<KeyWithdrawVotePostError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
